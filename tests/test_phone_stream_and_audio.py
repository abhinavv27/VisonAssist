...
- PhoneStreamCamera URL normalization, non-blocking socket probing, standby HUD.
- Threaded zero-latency frame grabber and clean release.
- AudioStreamServer & _PhoneAudioHTTPHandler endpoints (/phone-audio, /api/speech).
...
class TestPhoneStreamAdapter:
    ...
    def test_url_normalization(self):
        # Plain IP:port
        assert normalize_stream_url("192.168.43.1:8080") == "http://192.168.43.1:8080/video"
        
        # http:// without /video
        assert normalize_stream_url("http://192.168.1.100:8080") == "http://192.168.1.100:8080/video"
        
        # Already full video URL
        assert normalize_stream_url("http://10.0.0.5:8080/video") == "http://10.0.0.5:8080/video"
        
        # RTSP stream URL
        assert normalize_stream_url("rtsp://192.168.1.20:8554/live") == "rtsp://192.168.1.20:8554/live"

    def test_probe_stream_connectivity_unreachable(self):
        # Unreachable local IP returns False quickly without hanging
        t0 = time.perf_counter()
        reachable = probe_stream_connectivity("http://192.0.2.1:8080/video", timeout=0.3)
...
class TestPhoneAudioRelay:
    """Tests smartphone browser audio endpoint and broadcast synchronization."""
    ...
    def test_phone_audio_http_endpoints(self):
        ...
        try:
            ...
            with urllib.request.urlopen(ui_url, timeout=2.0) as resp:
                assert resp.status == 200
    ...
    def test_priority_emergency_broadcast_metadata(self):
        ...
        time.sleep(0.2)
        try:
            tts.speak("Warning. Collision hazard detected.", interrupt=True, priority=True)
            time.sleep(0.2)

            with _SPEECH_LOCK:
                assert _LATEST_SPEECH["text"] == "Warning. Collision hazard detected."
